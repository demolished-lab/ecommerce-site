...
from app.schemas.product_schema import (
    ProductCreate, ProductUpdate, ProductDetailResponse, ProductListItem,
    CategoryResponse, CategoryTreeResponse, ProductFilters, ProductReviewCreate,
...
from app.middleware.auth_middleware import get_current_user, get_current_user_optional, require_role
...
def list_products(
    ...
    sort_by: str = Query("created_at", pattern="^(created_at|price|popularity|rating|name)$"),
...
@router.get("/{product_id}/reviews", response_model=ApiResponse[List[ProductReviewResponse]])
def get_reviews(
    product_id: UUID,
    page: int = Query(1, ge=1),
...
def upload_product_images(
    ...
    return SuccessResponse(
        success=True,
        message=f"{len(files)} images uploaded"
    )
